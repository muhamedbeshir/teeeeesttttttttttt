...
import { User, Lock, Baby, Users, ArrowRight, Mail } from 'lucide-react'
    ...
    <>
      ...
      <main className="flex-grow min-h-screen relative overflow-hidden bg-gradient-to-br from-[#fdfbf7] via-[#f5efe6] to-[#e6dcc6]">
        ...
        <section className="relative z-10 min-h-[85vh] flex items-center justify-center p-4 sm:p-6 fade-in">
          <div className={`bg-white/70 backdrop-blur-xl p-8 sm:p-10 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] w-full max-w-md border ${error ? 'border-red-300' : success ? 'border-green-300' : 'border-white/50'} relative overflow-hidden transition-all duration-300 ${shake ? 'animate-shake' : ''}`}>
            ...
            <div className="text-center mb-8">
              ...
              <h2 className="text-2xl sm:text-3xl font-extrabold text-[#603814] tracking-tight">
                {isChild ? 'إنشاء حساب طفل' : 'إنشاء حساب ولي أمر'}
              </h2>
              <p className="text-[#603814] mt-2 font-medium opacity-80">
                {isChild ? 'سجل حساب جديد للطفل' : 'سجل حساب جديد لولي الأمر'}
              </p>
            </div>

            {success && (
              <div className="bg-green-50 border border-green-200 text-green-600 text-sm rounded-lg p-3 text-center font-medium mb-4">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="space-y-2">
                <label className="text-sm font-semibold text-[#603814] mr-1">تأكيد كلمة المرور</label>
                <div className="relative group">
                  <div className={`absolute right-3 top-3.5 transition-colors duration-300 ${isChild ? 'group-focus-within:text-orange-500' : 'group-focus-within:text-indigo-500'} text-[#c4996c]`}>
                    <Lock className="w-5 h-5" />
                  </div>
              ...
              <button 
                ...
                disabled={success}
              >
                {success ? 'جاري التحويل...' : 'إنشاء حساب'}
              </button>

              <div className="text-center">
                <p className="text-[#603814] text-sm mb-2">لديك حساب بالفعل؟</p>
                <Link 
                  to="/login" 
                  state={{ userType }}
